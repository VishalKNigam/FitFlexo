...
export default function TrainersList() {
  ...
  function fetchData(search, select) {
    ...
    axios
      .get(
        `https://fitness-mkwg.onrender.com/trainers?q=${search}&_sort=name&_order=${select}`
      )
      .then((res) => {
        setData(res.data);
        setLoading(false);
  ...
  const handleDelete = async (TrainerId) => {
    setLoading(true);
    await axios
      .delete(`https://fitness-mkwg.onrender.com/trainers/${TrainerId}`)
      .then((res) => {
        console.log(res);
        setLoading(false);
        setFlag(!flag);
          ...
          {data.length > 0 &&
            data.map((e) => (
              ...
              >
                <div className="w-25">
                  <img
                    className="card-img mb-2 rounded h-100"
                    src={e.image}
                    alt=""
                  />
                </div>
